...
export default function VisaDashboard() {
  ...
  const fetchVisas = async () => {
    ...
    // Filter ONLY parent visas (no parentSlug or empty parentSlug)
    const parentVisas = data.filter(visa => !visa.parentSlug || visa.parentSlug === "");
    setVisas(parentVisas);
  };
    ...
    <div className="p-6 max-w-7xl mx-auto bg-white rounded-lg shadow-md">
      <form onSubmit={handleSubmit} className="space-y-6 bg-gray-50 p-6 rounded-lg shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block font-semibold mb-1" htmlFor="parentSlug">Parent Slug (for children)*</label>
            <select
              name="parentSlug"
              id="parentSlug"
            ...
            >
              <option value="">Create Parent Visa</option>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              type="text"
              value={form.parentSlug ? `/migrate/${form.parentSlug}/${form.slug}` : `/migrate/${form.slug}`}
      ...
      <h2 className="text-xl font-semibold my-6 text-center text-gray-800">Parent Visas Only</h2>
      {visas.length === 0 ? (
        <p className="text-center text-gray-500">No parent visas added yet.</p>
      ) : (
