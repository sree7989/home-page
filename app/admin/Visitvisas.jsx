...
export default function InvestorVisaDashboard() {
  ...
  useEffect(() => {
    fetch("/api/visitvisas") // <-- updated URL
      .then((res) => res.json())
      .then(setVisas);
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    await fetch("/api/visitvisas", { // <-- updated URL
      method,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(form),
    });
    const updated = await fetch("/api/visitvisas").then((r) => r.json()); // <-- updated URL
    setVisas(updated);
    resetForm();
  };
    ...
    <div className="p-6 max-w-7xl mx-auto bg-white rounded-lg shadow-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-gray-50 p-6 rounded-lg shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block font-semibold mb-1" htmlFor="name">Visit Visa Name*</label>
            <input
              type="text"
              name="name"
              id="name"
              placeholder="Visit Visa Name"
              value={form.name}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input
              ...
              name="metaKeywords"
              id="metaKeywords"
              placeholder="e.g. visit visa, business, investment"
        ...
        >
          {editingIndex !== null ? "Update Visit Visa" : "+ Add Visit Visa"}
        </button>
      </form>

      <h2 className="text-xl font-semibold my-6 text-center text-gray-800">All Visit Visas</h2>
      {visas.length === 0 ? (
        <p className="text-center text-gray-500">No visit visas added yet.</p>
      ) : (
